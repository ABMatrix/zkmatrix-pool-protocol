...
fn test_decode() {
    use rand;
    use rand::RngCore;
    use snarkvm::algorithms::{
        fft::{DensePolynomial, EvaluationDomain, Evaluations},
        polycommit::kzg10::KZGProof,
    };
    use snarkvm::curves::bls12_377::{fr, Bls12_377};
    use snarkvm::prelude::{
        Address, EpochChallenge, FromBytes, PrivateKey, PuzzleConfig, Testnet3, ToBytes, Uniform,
    };
    ...
    let epoch_challenge =
        EpochChallenge::<Testnet3>::new(rng.next_u32(), Default::default(), degree).unwrap();
    println!(
        "epoch_block_hash: {}",
        epoch_challenge.epoch_block_hash().to_string()
    );

    let d = epoch_challenge.degree();

    let epoch_challenge_2 = EpochChallenge::<Testnet3>::new(
        epoch_challenge.epoch_number(),
        epoch_challenge.epoch_block_hash(),
        d,
    )
    .unwrap();
    ...
    let evaluations_s = epoch_challenge
        .epoch_polynomial_evaluations()
        .evaluations
        .iter()
        .map(|e| e.to_string())
        .collect::<Vec<String>>();
    let size = epoch_challenge
        .epoch_polynomial_evaluations()
        .domain()
        .size
        .to_string();
    let log_size_of_group = epoch_challenge
        .epoch_polynomial_evaluations()
        .domain()
        .log_size_of_group;
    let size_as_field_element = epoch_challenge
        .epoch_polynomial_evaluations()
        .domain()
        .size_as_field_element
        .to_string();
    let size_inv = epoch_challenge
        .epoch_polynomial_evaluations()
        .domain()
        .size_inv
        .to_string();
    let group_gen = epoch_challenge
        .epoch_polynomial_evaluations()
        .domain()
        .group_gen
        .to_string();
    let group_gen_inv = epoch_challenge
        .epoch_polynomial_evaluations()
        .domain()
        .group_gen_inv
        .to_string();
    let generator_inv = epoch_challenge
        .epoch_polynomial_evaluations()
        .domain()
        .generator_inv
        .to_string();
    ...
    assert_eq!(
        evaluations1,
        epoch_challenge.epoch_polynomial_evaluations().clone()
    );
    ...
    let epoch_challenge_2 = EpochChallenge::<Testnet3>::from_bytes_le(
        convert_to_u8(&ConvertType::EpochChallenge(epoch_challenge_s))
            .unwrap()
            .as_slice(),
    )
    .unwrap();
    assert_eq!(epoch_challenge, epoch_challenge_2);
    let address_2 = Address::<Testnet3>::from_bytes_le(
        convert_to_u8(&ConvertType::Address(address_s))
            .unwrap()
            .as_slice(),
    )
    .unwrap();
    assert_eq!(address, address_2);
    let result = CoinbasePuzzle::prove(
        &puzzle,
        &epoch_challenge,
        address,
        u64::rand(&mut rng),
