...
pub fn parse_job_id(job_id: String) -> anyhow::Result<(u32, u32, String)> {
    ...
    let server_id = step1[2];

    Ok((height, epoch_num, server_id.to_string()))
}
...
fn test_get_height_and_epoch_num() {
    ...
    let job_id = new_job_id(height_raw, epoch_num_raw, server_id.clone());
    println!("{}", &job_id);
    let (height, epoch_num, server_id_r) = parse_job_id(job_id).unwrap();
